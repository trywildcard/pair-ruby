...
describe "article card from valid url metatags" do
  ...
  article_card.populate_from_metatags('http://www.engadget.com/2013/09/09/lg-g2-review/')
  it "is built from json" do
    expect(article_card.web_url).to eql 'http://www.engadget.com/2013/09/09/lg-g2-review/' 
    expect(article_card.article.title).to eql 'LG G2 review' 
    expect(article_card.article.media.image_url).to eql 'http://www.blogcdn.com/www.engadget.com/media/2013/09/g2review-1378739225.jpg'
    expect(article_card.article.abstract_content).to eql 'There are a lot of smartphones out there now. You know this. To add to the confusion, many companies are now parading out multiple top-drawer phones: thi'
